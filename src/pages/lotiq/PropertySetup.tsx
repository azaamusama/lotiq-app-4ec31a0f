...
const steps = [
  ...
  { label: "Zones & Size", icon: MapPin },
  { label: "Use Cases", icon: Settings2 },
  { label: "Camera Estimate", icon: Video },
...
export default function PropertySetup() {
  ...
  return (
    <AppLayout title="Property Setup" subtitle="Configure your property">
      {/* Progress */}
      <div className="flex items-center gap-2 mb-8 max-w-3xl overflow-x-auto">
        {steps.map((s, i) => (
      ...
      <Card className="max-w-2xl">
        <CardContent className="p-6">
          ...
          {step === 1 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold">Parking Zones & Size</h3>
              <div className="space-y-3">
                ...
                {useCases.map(uc => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {selectedUseCases.includes(uc.id) && <CheckCircle2 className="h-4 w-4 text-primary" />}
                    </div>
                  </button>
                ))}
              </div>
          ...
          {step === 3 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold">Camera Estimation</h3>
              <div className="p-6 rounded-lg bg-muted/50 text-center">
                <Video className="h-8 w-8 text-primary mx-auto mb-3" />
                <p className="text-2xl font-bold font-mono-data">8 cameras</p>
                <p className="text-sm text-muted-foreground mt-1">Recommended for your property size and use cases</p>
                <p className="text-xs text-muted-foreground mt-3">Based on 120 spots, 4 zones, {selectedUseCases.length} active use cases</p>
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="space-y-4">
              ...
              <div className="p-6 rounded-lg border-2 border-primary">
                ...
                <p className="text-sm text-muted-foreground mb-4">Everything included. No hardware costs.</p>
                <ul className="space-y-2 text-sm">
                  {["All detection use cases", "Unlimited incidents", "Real-time monitoring", "Tow coordination", "Evidence storage", "One-time installation included"].map(f => (
                    <li key={f} className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-status-resolved" />{f}</li>
                  ))}
                </ul>
              </div>
            </div>
          ...
          {step === 5 && (
            <div className="space-y-4 text-center py-6">
              <div className="w-16 h-16 rounded-full bg-status-resolved/10 flex items-center justify-center mx-auto">
                <Rocket className="h-8 w-8 text-status-resolved" />
              </div>
